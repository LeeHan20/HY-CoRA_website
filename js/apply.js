document.addEventListener('DOMContentLoaded', () => {
    const navButtons = document.querySelectorAll('.about-nav .nav-button');

    navButtons.forEach((button) => {
        button.addEventListener('click', (event) => {
            event.preventDefault(); // 기본 링크 이동 방지

            // 모든 버튼에서 'active' 클래스 제거, 클릭된 버튼에만 추가
            navButtons.forEach((btn) => btn.classList.remove('active'));
            button.classList.add('active');

            // data-target 속성에서 스크롤할 대상 ID 가져오기
            const targetId = button.getAttribute('data-target');
            const targetSection = document.getElementById(targetId);

            if (targetSection) {
                // 부드러운 스크롤 적용
                targetSection.scrollIntoView({
                    behavior: 'smooth',
                    block: 'start', // 섹션의 시작 부분으로 스크롤
                });
            }
        });
    });
});
// FAQ 하이퍼링크 클릭 시 FAQ 섹션으로 스크롤
const faqLink = document.getElementById('FAQ-hyperlink');
if (faqLink) {
    faqLink.addEventListener('click', () => {
        const faqSection = document.getElementById('faq');
        if (faqSection) {
            faqSection.scrollIntoView({
                behavior: 'smooth',
                block: 'start',
            });
        }
    });
}
document.addEventListener('DOMContentLoaded', () => {
    // 이미 네가 갖고 있는 about-nav 버튼 스크립트는 그대로 두고
    // 밑에 추가 코드만 넣으면 돼!

    // 공통 스크롤 함수
    function scrollToId(targetId) {
        const section = document.getElementById(targetId);
        if (section) {
            section.scrollIntoView({
                behavior: 'smooth',
                block: 'start',
            });
        }
    }

    // "모집 일정" 텍스트 클릭 시 Schedule 섹션으로 이동
    const scheduleLink = document.getElementById('apply_schedule');
    if (scheduleLink) {
        scheduleLink.addEventListener('click', () => {
            scrollToId('Schedule');
        });
    }

    // "문의" 텍스트 클릭 시 contact 섹션으로 이동
    const inquiryLink = document.getElementById('inquiry');
    if (inquiryLink) {
        inquiryLink.addEventListener('click', () => {
            scrollToId('contact');
        });
    }
});
